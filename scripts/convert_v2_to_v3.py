...
class V2ToV3Converter:
    ...
    def convert_vault_policies_to_policies(self, vault_policies: Dict[str, str]) -> Dict[str, Dict[str, Union[str, int]]]:
        ...
        for policy_name, policy_content in vault_policies.items():
            ...
            charset_rules = re.findall(r'rule\s+"charset"\s*{\s*charset\s*=\s*"([^"]+)"', policy_content)

            if not charset_rules:
                policy['charset'] = 'alphanumeric'
                self.log(f"No charset rules found in policy '{policy_name}', using default 'alphanumeric'")
            else:
                # Analyze charset rules to determine v3 charset type
                has_lower = any('abcdefghijklmnopqrstuvwxyz' in rule for rule in charset_rules)
                has_upper = any('ABCDEFGHIJKLMNOPQRSTUVWXYZ' in rule for rule in charset_rules)
                has_digits = any('0123456789' in rule for rule in charset_rules)
                has_symbols = any(re.search(r'[!@#$%^&*()_+\-=\[\]{}|;\':",./<>?]', rule) for rule in charset_rules)
                ...
                if has_symbols:
                    ...
                    policy['charset'] = 'alphanumeric'
                    self.log(f"Could not determine charset for policy '{policy_name}', using default 'alphanumeric'")
    ...
    def convert_backing_store(self, backing_store: str) -> str:
        ...
        if result != backing_store.lower():
            self.log(f"Converted backingStore '{backing_store}' to secretstore '{result}'")

        return result

    def convert_field_to_v3(self, field: Dict[str, Any], vault_policies: Dict[str, str]) -> tuple[str, Any]:
        ...
        if 'path' in field:
            ...
            if field.get('base64', False):
                ...
                self.log(f"Field '{field_name}' converted to base64 file reference: {file_ref}")
            else:
                file_ref = f"file://{path}"
                self.log(f"Field '{field_name}' converted to file reference: {file_ref}")
        ...
        if field.get('onMissingValue') == 'generate':
            ...
            if vault_policy:
                ...
                self.log(f"Field '{field_name}' converted to generation reference: {generate_ref}")
                return field_name, generate_ref
            else:
                self.log(f"Field '{field_name}' has generate but no vaultPolicy, using 'generate:basic'")
    ...
    def convert_secrets(self, v2_secrets: List[Dict[str, Any]], vault_policies: Dict[str, str]) -> Dict[str, Dict[str, Any]]:
        ...
        for secret in v2_secrets:
            ...
            if 'fields' in secret:
                for field in secret['fields']:
                    field_name, field_value = self.convert_field_to_v3(field, vault_policies)
            ...
            if 'vaultPrefixes' in secret:
                ...
                self.log(f"Secret '{secret_name}' vaultPrefixes converted to targets: {secret['vaultPrefixes']}")

            # Handle vaultMount (note: v3 doesn't have direct equivalent, log warning)
            if 'vaultMount' in secret:
                self.log(f"WARNING: Secret '{secret_name}' has vaultMount '{secret['vaultMount']}' - this is not directly supported in v3.0")
    ...
    def extract_global_settings(self, v2_secrets: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def convert_file(self, input_path: str) -> Dict[str, Any]:
        ...
        self.log(f"Converting values-secret file from version 2.0 to 3.0")
        ...
        if vault_policies:
            v3_data['policies'] = self.convert_vault_policies_to_policies(vault_policies)

        # Extract global settings
        secrets_list = v2_data.get('secrets', [])
        global_settings = self.extract_global_settings(secrets_list)
        if global_settings:
            v3_data['settings'] = global_settings
            self.log(f"Added global settings: {global_settings}")

        # Convert secrets (always include secrets section, even if empty)
        v3_data['secrets'] = self.convert_secrets(secrets_list, vault_policies)

        self.log(f"Conversion completed successfully. Converted {len(secrets_list)} secrets.")
    ...
    def write_output(self, v3_data: Dict[str, Any], output_path: str) -> None:
        ...
        try:
            with open(output_path, 'w') as f:
                ...
                yaml.dump(v3_data, f, default_flow_style=False, sort_keys=False, indent=2)
...
def main():
    ...
    parser.add_argument('output', nargs='?', help='Output v3.0 YAML file path (default: input_v3.yaml)')
    parser.add_argument('--quiet', '-q', action='store_true', help='Suppress conversion messages')
    parser.add_argument('--summary', '-s', action='store_true', help='Show conversion summary')
